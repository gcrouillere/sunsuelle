...
function OnloadFunction() {
 ...
 document.querySelectorAll('#index_table_produits tbody tr').forEach(
  ...
  // PRODUCT FIELD UPDATE ON BLUR
   var editableCells = Array.from(document.querySelectorAll("td.col.col-name, td.col.col-description, td.col.col-stock, td.col.col-weight,td.col.col-price_cents, td.col.col-category_ids"));
...
updatePositionsInDB = (startingPosition) => {
  ...
  $.ajax({
    type: "GET",
    url: `${urlRoot}/sacs/update_positions_after_swap_in_admin?finalPositions=[${rowsFinalPositions}]&startingPosition=${startingPosition}`,
    dataType: "JSON"
  }).done((data) => {
    console.log("done");
    OnloadFunction();
...
clickOnEditableCell = (event) => {
  ...
  if (Array.from(cell.classList).indexOf("editing") == -1 && cell.nodeName == "TD") {
    ...
    input.addEventListener('blur', function(event) {
      ...
      newValue = updatingField == 'category_ids' ? Array.from(this.selectedOptions).map(x=> x.innerHTML).join("#%#") : this.value;

      //VALIDATION
      if (validateField(newValue, inputType)) {
        //DOM UPDATE
        if (updatingField == 'category_ids') {
            relatedCell.innerHTML = newValue.split("#%#").join(" - ")
        } else {
...
inputConstruction = (updatingField, value) => {
  if (updatingField != 'category_ids') {
    ...
    return `<select multiple="multiple" class="editingInput ${updatingField}">${
